...
- `jest.mock()` は **トップレベル**（`import` 文と同じ階層）で宣言する必要があります。関数の中などで呼び出すと、モックが正しく反映されないことがあります。
- **ESモジュールのエクスポートは `readonly` のため、`exportedFunction = jest.fn()` のように直接上書きすることはできません。** そのため `jest.mock()` による事前のモック化が必要です。
...
console.log(currentJST()); // => 常に同じ Date を返す
```
...
```typescript
// masterPrisma.mTest.findFirst をモック化する場合
...
```
- Prisma Client のような構造が深く、自動生成された型を持つライブラリでは、単純な `jest.mock()` では意図したモックが機能しないことがあります。
- 第 2 引数でモジュールの構造を明示的に定義することで、モックの適用ミスや TypeScript の型エラーを回避できます。
...
- `jest.spyOn(obj, "method")` は、対象オブジェクトのメソッドに対して呼び出し監視（spy） を行うと同時に、必要に応じてモック実装に差し替えることもできます。
- デフォルトでは元の関数の動作を保持しますが、`.mockImplementation(fn)` を使うことで任意の処理に差し替え可能です。
> **使い分けの判断基準としては、**
> - グローバル関数をモックや監視するときは、`jest.spyOn` で一時的に差し替え、あとで元に戻す。
> - 単純に関数を置き換えっぱなしにしたい（元に戻さなくて良い）場合は `jest.fn()` を使う。

### テスト間・テスト内でモック関数をリセットする方法
...
afterEach(() => {
  // すべてのモック関数の実装と呼び出し履歴をリセット
  jest.resetAllMocks();
});
```

- Jest のモック関数（`jest.fn()`）は、テスト間で状態（戻り値や呼び出し履歴など）を共有します。
- テストの独立性を保つためには、各テストの前後でモックをリセットするのが安全です。
- よく使われるリセットメソッドは以下の通り：
  - `jest.resetAllMocks()`  
    - モック関数の実装（戻り値や実装内容）と呼び出し履歴をリセットし、初期状態の `jest.fn()` に戻します。  
    - spy で上書きした実装はリセットされますが、元の実装に戻るわけではありません。  
      （元の関数に戻すには `mockRestore()` が必要です。）
  - `jest.clearAllMocks()`  
    - 呼び出し履歴のみをリセットし、モック関数や spy の実装は維持します。
  - `jest.restoreAllMocks()`  
    - spy で差し替えたすべての関数のモック化を解除し、元の実装に戻します。  
    - `mockRestore()` をまとめて行うイメージで、spy に対してのみ効果があります。
  - 個別にモック関数のリセットも可能で、特定のモックだけ呼び出し履歴や実装をクリアしたい場合に使います（例：`mockFn.mockClear()` や `mockFn.mockReset()`）。
  - spy で差し替えた関数を元の実装に戻す場合は、`mockFn.mockRestore()` を使用します。  
    これは spy のモック化を解除して元の実装に復帰させるためのメソッドで、`restoreAllMocks()` とは異なり個別に制御可能です。
