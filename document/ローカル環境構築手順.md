...
- **`generator`**  
  ...
  デフォルトでは `node_modules/.prisma/client` にクライアントコードが生成されます。
- **`datasource`**  
...
この設定は、`DATABASE_URL_TRANSACTION` の環境変数を使って、MySQLデータベースに接続するデータソース（`db`）を定義しています。
...
Prisma の MySQL スキーマ定義で

```sql
DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
```

のような 「作成時に自動で現在時刻を入れ、更新時にも自動でタイムスタンプを更新する」 の属性を付与することができません。

Prisma の `@updatedAt` はアプリ側（UTC）で日時をセットするため、MySQL の JST 設定とずれが出ます。  
そのためマイグレーションファイルを作成したら SQL ファイルを直接編集し、全テーブルの update_at カラムに `DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP` を追加します。  
この設定により Prisma スキーマでは @updatedAt が不要になります。
...
$ echo -e 'DATABASE_URL_MASTER="mysql://pme_user:pmepassword@mysql:3306/pme_master"\nDATABASE_URL_TRANSACTION="mysql://pme_user:pmepassword@mysql:3306/pme_transaction"' > admin/.env

# frontend の .env を作成（なければ）
$ ll frontend/.env
ls: cannot access 'frontend/.env': No such file or directory
$ echo -e 'DATABASE_URL_MASTER="mysql://pme_user:pmepassword@mysql:3306/pme_master"\nDATABASE_URL_TRANSACTION="mysql://pme_user:pmepassword@mysql:3306/pme_transaction"' > frontend/.env
```

これらのコマンドで、`admin` と `frontend` に Prisma 用の `.env` ファイルを作成し、MySQL の接続先を設定しています。   
Prisma は `.env` の `DATABASE_URL_MASTER` と `DATABASE_URL_TRANSACTION` を使って、MySQL コンテナ に接続します。
...
# admin コンテナでマイグレーションの適用と Prisma Client の作成
...
# frontend コンテナでは、常に Prisma Client のみを作成
...
作成したマイグレーションファイルの所有者が root ユーザになっているため、Ubuntu のログインユーザがファイルを削除や編集できない可能性があります。  
そのため、`sudo chown` コマンドを使って、所有者をログインユーザに変更しておいてください。

## サーバコンポーネントと API の確認

下記 URL にアクセスしてエラーが発生しないことを確認してください。
...
任意の SQL クライアントツールから以下の情報で接続し、`pme_master` と `pme_transaction` のデータベースおよびそれぞれのテーブルが存在することを確認してください。
