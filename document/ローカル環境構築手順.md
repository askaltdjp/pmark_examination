...
model MTest {
  ...
  createAt    DateTime  @default(dbgenerated("CURRENT_TIMESTAMP(3)")) @map("create_at")
  updateAt    DateTime  @default(dbgenerated("CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3)")) @map("update_at")
...
model TTest {
  ...
  result     Int
  testAt     DateTime  @default(dbgenerated("CURRENT_TIMESTAMP(3)")) @map("test_at")
  createAt   DateTime  @default(dbgenerated("CURRENT_TIMESTAMP(3)")) @map("create_at")
  updateAt   DateTime  @default(dbgenerated("CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3)")) @map("update_at")
  deleteAt   DateTime? @map("delete_at")
...
# (テーブル変更者のみ) admin コンテナでマイグレーションファイルの作成と適用、Prisma Client の作成
...
# (テーブル変更者のみ) admin コンテナでマイグレーションファイルのみを作成
...
# (マイグレーション実行者のみ) admin コンテナでマイグレーションの適用と Prisma Client の作成
...
# (共通) frontend コンテナでは、常に Prisma Client のみを作成
