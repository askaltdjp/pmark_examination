...
/**
 * 試験一覧と各試験の集計結果を取得するサービス関数
 *
 * @returns オブジェクト（試験マスタ配列、試験IDごとの受験者・合格者情報のRecord）
 */
export async function listService(): Promise<{ mTests: MTest[], testSummary: TestSummaryRecord }> {
    // すべての試験マスタを取得
    const mTests = await MTestRepository.findAll();
    mTests.sort((a, b) => a.startAt.getTime() - b.startAt.getTime()); // 開始日時で昇順ソート

    // 試験IDごとに、受験者数（employeeIdのユニーク数）および合格者数（result = 1の合計）を集計
