...
export async function downloadService(
    ...
    for (let i = 0; i < MAX_QUESTIONS; i++) {
        ...
        // セルM[奇数行番号]に解答を書き込む
        worksheet.getCell(`M${startRow + i * 2}`).value = tTestAnswer ? (tTestAnswer.answer ? "Yes" : "No") : "";
        // セルP[奇数行番号]に正解を書き込む
        worksheet.getCell(`P${startRow + i * 2}`).value = tTestAnswer ? (tTestAnswer.answer === mTestQuestion?.correct ? "〇" : "✕") : "";
    }
